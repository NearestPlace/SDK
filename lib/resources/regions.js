...
const resolver = {
  ...
  getByNameAndPoint(data) {
    return data.getRegionByNameAndPoint;
  },
  getByPath(data) {
...
export default class SDKResourceApp {
  ...
  getByNameAndPoint(options, callback) {
    const { url, apiKey } = this;
    return ApiExecute(
      `{ ${this.getByNameAndPointQuery(options)} }`,
      resolver.getByNameAndPoint,
      url,
      apiKey,
      callback,
    );
   ...
   * Get raw query
   */
  getByNameAndPointQuery(options) {
    const { app } = this;
    const { name, lat, lng, lang, fields = ['id'] } = options;
    return Query('getRegionByNameAndPoint', { name, lat, lng, lang }, fields);
  }
}
